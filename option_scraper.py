...
driver = webdriver.Chrome('C:/Users/PSYDUCK/PycharmProjects/Caculate_Finance/chromedriver.exe')
...
def scrapeData(startIndex, bs, rf, wait, verbose=True):
    ...
    Inputs:
        ...
        wait       - wait period between page driver
    """
    ...
    for i, ticker in enumerate(tickers[startIndex:startIndex + bs]):
        ...
        maturity = (dates[0] - unixToday) / (
                    60 * 60 * 24 * 365.25)  # Convert UNIX time difference to fraction of a year
        if maturity <= 0:
        ...
        for date in dates[1:]:
            ...
            url = "https://finance.yahoo.com/quote/" + ticker + "/options" + "?date=" + str(
                date)  # Special URL for future dates

            if verbose:
                print(url)

            maturity = (date - unixToday) / (
                        60 * 60 * 24 * 365.25)  # Convert UNIX time difference to fraction of a year
